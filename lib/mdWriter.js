...
module.exports = function (md, apiName, apiURL, routes) {
	...
	md.push({ h1: 'API name: ' + apiName });
	md.push({ p: 'API URL: ' + apiURL });
	...
	var json = function (val, title) {
		...
		var table = [];
		if (val.items) {
			if (title) para('Items: **' + title + '**');
			else para('Items:');
			table = [['Type', 'Description']];
			var desc = [val.items.type];
			_typeL = val.items.type.toLowerCase();
			if (_typeL === 'object' || _typeL === 'array') {
				nextTable.push({name: title, data: val.items});
			}
			if (val.items.description) desc.push(val.items.description);
			...
			if (val.required) required = val.required;
			...
			for (var name in props) {
				...
				if (props[name].description) p[name].Description = props[name].description;
				else p[name].Description = '';
				...
				keys.forEach(function (k) {
					if (k !== 'type' && k !== 'description' && k !== 'properties' && k !== 'required' && k !== 'items') {
						if (paramsAdd.indexOf(k) === -1) paramsAdd.push(k);
					}
				});
			}
	...
	return md;
	// var mdString = json2md(md).replace(/\|\n\n\|/gm, '|\n|');
