...
module.exports = function(config,mode,logger,server,emitMessage){
	...
	var routing = function(req, res, next) {
		if(routes[req.url]){
			...
			return routes[req.url](req, res, next, restify, faker);
		} else {
	...
	return {
		start : function(){
			stalker = watchr.open(config.sources, listener, next);
		...
		case 'themes':
			if(!data.isDir && data.name==config.defaultTheme){
				if(data.pathArr[2]){
					...
					} else if(data.pathArr[2]=='vars'){
						compil.mainCss(config,mode,config.defaultTheme,toPuts,function(){
							compilCpMain(config,mode,config.defaultTheme,toPuts,function(){
								config.pages = fs.readdirSync(config.sourcesPages);
